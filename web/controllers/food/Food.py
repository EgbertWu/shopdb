...
@route_food.route("/promotion-ops", methods=["POST"])
def promotion():
    resp = {'code': 200, 'msg': '操作成功~~', 'data': {}}
    req = request.values
    ...
    if act not in ['recomm', 'delrecomm']:
        resp['code'] = -1
        resp['msg'] = "操作有误，请重试~~"
        return jsonify(resp)

    cnt = Food.query.filter_by(recomm=1).count()
    idlist = list(id.split(','))
    if act == 'recomm':
        if cnt > 4:
            resp['code'] = -1
            resp['msg'] = "推广产品数已经达到上限,请重试~~"
            return jsonify(resp)
        for i in idlist:
            food_info = Food.query.filter_by(id=i).first()
            if food_info.recomm == 1:
                resp['code'] = -1
                resp['msg'] = '勾选产品已参与推广，请重试~~'
                return jsonify(resp)
            food_info.recomm = 1
            food_info.updated_time = getCurrentDate()
    elif act == 'delrecomm':
        for i in idlist:
            food_info = Food.query.filter_by(id=i).first()
            if food_info.recomm == 0:
                resp['code'] = -1
                resp['msg'] = '勾选产品未进行推广，请重试~~'
                return jsonify(resp)
            food_info.recomm = 0
            food_info.updated_time = getCurrentDate()
...
@route_food.route("/clearfood", methods=["POST"])
def clearfood():
    resp = {'code': 200, 'msg': '操作成功~~', 'data': {}}
    req = request.values
    ...
    idlist = list(id.split(','))
    for i in idlist:
        Food.query.filter_by(id=i).delete()
