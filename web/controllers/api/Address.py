...
from common.libs.Helper import getCurrentDate,getAddressCoord,getDistance
...
def myAddressSet():
	...
	# 只允许三公里距离
	if province_str not in district_str:
		user_addr = province_str
		if city_str not in district_str:
			user_addr += city_str
	user_addr += district_str + address
	user_loc = getAddressCoord(user_addr)
	shop_loc = getAddressCoord(app.config['MINA_APP']['shoplocation'])

	user_location = user_loc['result']['location']
	shop_location = shop_loc['result']['location']
	from_lng = shop_location['lng']
	from_lat = shop_location['lat']
	to_lng = user_location['lng']
	to_lat = user_location['lat']
	app.logger.info(from_lng)
	app.logger.info(from_lat)
	app.logger.info(to_lng)
	app.logger.info(to_lat)
	app.logger.info(type(from_lng))
	distance = getDistance(from_lat,from_lng,to_lat,to_lng)

	if int(distance) == -1 or int(distance) > 3000:
		resp['code'] = -1
		resp['msg'] = "地址已超出配送范围，请重新输入~~"
		return jsonify(resp)

	# {'message': 'query ok', 'result': {'location'dd: {'lat': 39.883385, 'lng': 116.375755}
