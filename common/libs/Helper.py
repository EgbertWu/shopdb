...
def getAddressCoord(user_addr):
    config_mina = app.config['MINA_APP']
    url = "https://apis.map.qq.com/ws/geocoder/v1/?address={0}&key={1}".format(user_addr, config_mina['mapkey'])
    r = requests.get(url=url)
    ret = json.loads(r.text)
    return ret


def getDistance(shop_lat, shop_lng, user_lat, user_lng):
    config_mina = app.config['MINA_APP']
    url = "https://apis.map.qq.com/ws/distance/v1/?mode=driving&from={0},{1}&to={2},{3}&key={4}".format(shop_lat, shop_lng, user_lat, user_lng, config_mina['mapkey'])
    r = requests.get(url=url)
    ret = json.loads(r.text)

    distance = ret['result']['elements'][0]['distance']
    return distance
