...
def foodIndex():
    ...
    #前端幻灯图片的的展示
    food_list = Food.query.filter_by( recomm = 1 ).order_by( Food.month_count.desc(),Food.id.desc() ).all()
...
def foodSearch():
    ...
    food_list = query.order_by(Food.month_count.desc(), Food.id.desc()).offset( offset ).limit( page_size ).all()
...
def foodInfo():
    ...
    resp['data']['info'] = {
        ...
        "month_count":food_info.month_count,
