...
'''
python manager.py runjob -m stat/daily -a member|food|site|food_month -p 2020-05-01
'''
...
class JobTask():
    ...
    def statFood(self, params):
        ...
        if not stat_food_list:
            app.logger.info("no data stat_food_list")
            return
    ...
    '''
    food_month统计,所有美食的月销售数据
    '''

    def foodMonth(self, params):
        act = params['act']
        date = params['date']
        date_from = params['date_from']
        date_to = params['date_to']
        ...
        date_from1 = now + datetime.timedelta(days=-30)
        date_from1 = getFormatDate(date_from1, format("%Y-%m-%d")) + " 00:00:00"
        app.logger.info("act:{0},from:{1},to:{2}".format(act, date_from1, date_to1))
        stat_food_list1 = db.session.query(FoodSaleChangeLog.food_id,
                                           func.sum(FoodSaleChangeLog.quantity).label("total_count")) \
            .filter(FoodSaleChangeLog.created_time >= date_from1, FoodSaleChangeLog.created_time <= date_to1) \
            .group_by(FoodSaleChangeLog.food_id).all()

        if not stat_food_list1:
            app.logger.info("no data month food")
            return

        # 更新月销数据
        for item in stat_food_list1:
            tmp_food_id = item[0]
            tmp_food = Food.query.filter_by(id=tmp_food_id).first()
            if tmp_food:
                tmp_model_food = tmp_food
                tmp_model_food.month_count = item[1]
                tmp_model_food.updated_time = getCurrentDate()
